...
const form = {
  bodySerializer: (body: Record<string, string | null | undefined>) =>
    new URLSearchParams(Object.entries(body).filter((entry): entry is [string, string] => entry[1] != null)),
...
export function createBauthClient(options: BauthClientOptions) {
  ...
  const api = createFetchClient<paths>({ baseUrl: options.baseUrl, fetch: options.fetch })

  async function exchangeCode(code: string, codeVerifier: string): Promise<Tokens> {
    const body = { grant_type: "authorization_code", client_id: clientId, code, redirect_uri: redirectUri, code_verifier: codeVerifier }
  ...
  return {
    ...
    async startLogin(state?: string): Promise<LoginFlow> {
      ...
      return { flowId: flow.flow_id, methods: flow.methods, expiresAt: flow.expires_at, codeVerifier }
    },

    /** Password step: returns the code, then call `exchangeCode`. */
    async submitPassword(flowId: string, email: string, password: string): Promise<string> {
      const params = { path: { flow_id: flowId } }
      const result = await unwrap(api.POST("/flows/login/{flow_id}/password", { params, body: { email, password } }))
      return result.code
    },
    ...
    async confirmMagicLink(token: string): Promise<string> {
      return (await unwrap(api.POST("/magic-link/confirm", { body: { token } }))).code
    },
    ...
    async confirmMagicCode(flowId: string, code: string): Promise<string> {
      const params = { path: { flow_id: flowId } }
      return (await unwrap(api.POST("/flows/login/{flow_id}/magic-code", { params, body: { code } }))).code
    },
    ...
    async logout(refreshToken: string): Promise<void> {
      await unwrap(api.POST("/oauth/revoke", { body: { client_id: clientId, token: refreshToken }, ...form }))
    },
    ...
    async register(email: string, password: string): Promise<void> {
      await unwrap(api.POST("/registration", { body: { client_id: clientId, email, password } }))
    },
    ...
    async requestPasswordReset(email: string): Promise<void> {
      await unwrap(api.POST("/recovery", { body: { client_id: clientId, email } }))
    },
    ...
    async changePassword(accessToken: string, currentPassword: string, newPassword: string): Promise<void> {
      ...
      await unwrap(api.POST("/me/password", { headers: bearer(accessToken), body }))
    },

    /** Sends a confirmation link to the new address, confirmed with `confirmVerification`. */
    async changeEmail(accessToken: string, password: string, newEmail: string): Promise<void> {
      await unwrap(api.POST("/me/email", { headers: bearer(accessToken), body: { password, new_email: newEmail } }))
    },
    ...
    async revokeSession(accessToken: string, sessionId: string): Promise<void> {
      ...
      await unwrap(api.DELETE("/me/sessions/{session_id}", { headers: bearer(accessToken), params }))
    },

    async deleteAccount(accessToken: string, password: string): Promise<void> {
      await unwrap(api.DELETE("/me", { headers: bearer(accessToken), body: { password } }))
    }
  }
}
